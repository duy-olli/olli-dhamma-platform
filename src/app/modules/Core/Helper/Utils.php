...
class Utils extends EnHelper
{
    ...
    */
    ...
    // Send confirmation mail using mailgun
    public static function sendMailgun(string $to, string $subject, string $content, $data = [])
    {
        $mg = new Mailgun(getenv('MAILGUN_API_KEY'));
        $result = $mg->sendMessage(getenv('MAILGUN_DOMAIN'), [
            'from'    => getenv('MAILGUN_SENDER') . ' <noreply@'. getenv('MAILGUN_DOMAIN') .'>',
            'to'      => $to,
            'subject' => $subject,
            'html'    => $content,
            'v:type' => $data['type'],
    ...
    public static function sendmail(string $to, string $subject, string $content, $data = [])
    {
        $mg = new Mailgun(getenv('MAILGUN_API_KEY'));
        $result = $mg->sendMessage(getenv('MAILGUN_DOMAIN'), [
            'from'    => getenv('MAILGUN_SENDER') . ' <noreply@'. getenv('MAILGUN_DOMAIN') .'>',
